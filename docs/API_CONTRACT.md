...
- `GET /api/dashboard`
- `GET /api/activities` (supports optional `theme` and `difficulty` query filters)
...
- Activity entries include `theme` and `difficulty` fields, and `/api/activities` returns available `themes` and `difficulties` values.
